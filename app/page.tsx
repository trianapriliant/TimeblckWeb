...
export default function TimeblckPage() {
  ...
  const renderGrid = (dateKey: string) => {
    ...
    if (settings.blockShape === 'solid') {
      let previewBlock: ScheduleBlock | null = null;
      if (activeBlock && dragOverSlot?.dateKey === dateKey) {
        previewBlock = { ...activeBlock, startTime: dragOverSlot.slotIndex };
      }
        ...
        <div className="relative h-[72rem]"> {/* 144 slots * 0.5rem/slot */}
          ...
          {previewBlock && (
             <TimeSlot
              ...
              dateKey={dateKey}
              slotIndex={previewBlock.startTime}
              block={previewBlock}
              blockShape={settings.blockShape}
              onClick={() => {}}
              selection={null}
      ...
      return (
        <div key={hour} className="relative h-12 border-b grid grid-cols-6">
          {Array.from({ length: SLOTS_PER_HOUR }).map((_, i) => {
            ...
            // Check if the current slot is part of a block being dragged over
            let dragBlock = null;
            if (activeBlock && dragOverSlot?.dateKey === dateKey &&
                ...
                dragBlock = {
                    ...activeBlock,
                    startTime: dragOverSlot.slotIndex
                };
            }
            
            const blockOnSlot = dragBlock ?? scheduleForDate?.get(currentSlotIndex);
            ...
            return (
              <TimeSlot
                ...
                selection={selection}
                isSelectedForCreation={isSelectedForCreation}
                isHighlightedForDrag={false} // No longer needed
                onClick={handleBlockClick}
                onMouseDown={handleMouseDown}
                onMouseEnter={handleMouseEnter}
                isDragging={activeBlock?.id === blockOnSlot?.id && !dragBlock}
                isPreview={!!dragBlock}
              />
            );
          })}
