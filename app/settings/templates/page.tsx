...
export default function TemplatesPage() {
  ...
  React.useEffect(() => {
    setPageActions({
        title: 'Templates',
        description: 'Manage your recurring time blocks here. They will automatically appear on your daily schedule.',
        fab: {
            label: 'Add Template',
            action: handleAddTemplateClick,
        },
    });
    return () => setPageActions(null);
  }, [setPageActions, handleAddTemplateClick]);
        ...
        {recurringBlocks.length === 0 ? (
          <Card>
              <CardContent className="pt-6">
                <div className="text-center py-12 border-2 border-dashed rounded-lg">
                    ...
                    <p className="text-muted-foreground mt-2 max-w-sm mx-auto">Click "Add Template" to create your first recurring block.</p>
                     <Button onClick={handleAddTemplateClick} className="mt-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {recurringBlocks.map((template) => {
              ...
              const colorClasses = BLOCK_COLORS[template.color as keyof typeof BLOCK_COLORS] ?? BLOCK_COLORS.slate;
                     ...
                     <div className="flex items-center gap-2">
                      ...
                      {(template.startDate || template.endDate) && (
                        <div className="flex items-center gap-2 text-xs text-muted-foreground border-t pt-3">
                          ...
                          <span>
                            {template.startDate ? format(new Date(template.startDate), 'MMM d') : 'Perpetual'}
                            {' - '}
                            {template.endDate ? format(new Date(template.endDate), 'MMM d, yyyy') : 'Perpetual'}
                          </span>
                        </div>
                      )}
                  </CardContent>
          ...
          <ScrollArea className="h-72">
            <div className="p-1 space-y-2">
              {SUGGESTED_TEMPLATES.map((template, index) => {
                 ...
                 const colorClasses = BLOCK_COLORS[colorKey] ?? BLOCK_COLORS.slate;
                 return (
                    <button
                        key={index}
                    ...
                    >
                        <div className={cn("p-2 mt-1 rounded-lg w-10 h-10", colorClasses.solid)} />
                        <div>
